...
namespace POCOGeneratorUI
{
	public sealed partial class POCOGeneratorForm
	{
		...
		private GeneratorResults GeneratePOCOs(bool forceGeneratingPOCOs = false,
											   bool isExport = false,
											   [InstantHandle] Action<IGenerator> BeforeGeneratePOCOs = null,
											   [InstantHandle] Action<IGenerator> AfterGeneratePOCOs = null)
		{
			generatePOCOsFromCheckedTreeNodes = this.selectedTables.HasAny()
			...
			if (generatePOCOsFromCheckedTreeNodes)
			{
				selectedTables = new(this.selectedTables);
				selectedViews = new(this.selectedViews);
				selectedProcedures = new(this.selectedProcedures);
				selectedFunctions = new(this.selectedFunctions);
				selectedTVPs = new(this.selectedTVPs);
			}
			else
			{
			...
			if (chkComplexTypes.Checked && selectedTables.HasAny())
			{
				selectedComplexTypeTables =
					new(selectedTables.SelectMany(t => t.ComplexTypeTables).Distinct().OrderBy(t => t.Name));
			}
			...
			void databaseGenerating(object sender, DatabaseGeneratingEventArgs e)
			{
				e.Skip = !selectedDatabases.Contains(e.Database);
			}
			...
			{
				e.Skip = !selectedTables.Contains(e.Table);
			}
			...
			{
				e.Skip = !selectedComplexTypeTables.Contains(e.ComplexTypeTable);
			}
			...
			{
				e.Skip = !selectedViews.Contains(e.View);
			}
			...
			{
				e.Skip = !selectedProcedures.Contains(e.Procedure);
			}
			...
			{
				e.Skip = !selectedFunctions.Contains(e.Function);
			}
			...
			{
				e.Skip = !selectedTVPs.Contains(e.TVP);
			}
		...
		private TreeNode GetSelectedNode()
		{
			...
			{
				return selectedNode.Tag is ForeignKey
					? selectedNode.Parent.Parent
					: selectedNode.Tag is ForeignKeyColumn
									? selectedNode.Parent.Parent.Parent
									: selectedNode.Tag is IEnumerable<TableIndex>
													? selectedNode.Parent
													: selectedNode.Tag is TableIndex
																	? selectedNode.Parent.Parent
