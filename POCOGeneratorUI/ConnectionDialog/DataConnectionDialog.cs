...
{
	internal sealed partial class DataConnectionDialog
		: Form
	{
		private DbConnectionStringBuilder builder;

		private IRDBMSHandler Handler => (IRDBMSHandler)ddlRDBMS.SelectedItem;
		...
		private void txtUserName_TextChanged(object sender, EventArgs e) => txtConnectionString.Text = ConnectionString;

		private void txtPassword_TextChanged(object sender, EventArgs e) => txtConnectionString.Text = ConnectionString;

		private void ddlDatabases_TextChanged(object sender, EventArgs e) => txtConnectionString.Text = ConnectionString;
		...
		public RDBMS RDBMS => Handler.RDBMS;
		...
		{
			DbConnectionStringBuilder builder = Handler.GetConnectionStringBuilder(ConnectionString);
					...
					if (servers.Count > 0)
					{
						...
						if (string.IsNullOrEmpty(currentServer) == false)
						{
							for (int i = 0; i < servers.Count; i++)
							{
								...
								if (string.Compare(server, currentServer, StringComparison.OrdinalIgnoreCase) == 0)
								{
									serverIndex = i;
									break;
								}
