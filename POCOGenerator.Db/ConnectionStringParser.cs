	...
	public abstract class ConnectionStringParser : IConnectionStringParser
	{
		...
		public virtual bool Validate(string connectionString)
		{
			try
			{
				...
				Parse(connectionString, ref serverName, ref initialDatabase, ref userId, ref integratedSecurity);
		...
		private static readonly Regex connectionTimeoutRegex = new(@"Connect(?:ion)?\s*Timeout\s*=\s*\d+\s*;?", RegexOptions.Compiled);
