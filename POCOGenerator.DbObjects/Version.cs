...
namespace POCOGenerator.DbObjects
{
	public sealed class Version : IComparable, IComparable<Version>, IComparer, IComparer<Version>, IEquatable<Version>, IEqualityComparer<Version>, ICloneable
	{
		...
		private static readonly Regex regexVersion = new(@"(?<Major>\d+)(?:\.(?<Minor>\d+))?(?:\.(?<Build>\d+))?(?:\.(?<Revision>\d+))?", RegexOptions.Compiled);
		...
		{
			return CompareVersions(Major, Minor, Build, Revision, major, minor, build, revision) == 0;
		}
		...
		{
			int hash = 5381;
			hash = ((hash << 5) + hash) ^ Major;
			hash = ((hash << 5) + hash) ^ Minor;
			hash = ((hash << 5) + hash) ^ Build;
			hash = ((hash << 5) + hash) ^ Revision;
			return hash;
		}
		...
		private static int CompareVersions(Version v1, Version v2)
		{
			...
			return v1 is null ? -1 : CompareVersions(v1.Major, v1.Minor, v1.Build, v1.Revision, v2.Major, v2.Minor, v2.Build, v2.Revision);
		}

		private static int CompareVersions(Version v1, int v2Major)
		{
			return v1 is null ? -1 : CompareVersions(v1.Major, v1.Minor, v1.Build, v1.Revision, v2Major, 0, 0, 0);
		}

		private static int CompareVersions(int v1Major, Version v2)
		{
			return v2 is null ? 1 : CompareVersions(v1Major, 0, 0, 0, v2.Major, v2.Minor, v2.Build, v2.Revision);
		}
		...
		private static int CompareVersions(Version v1, double v2MajorMinor)
		{
			...
			int v2Major = Int32.Parse(segments[0]);
			int v2Minor = segments.Length == 2 ? Int32.Parse(segments[1]) : 0;
		...
		private static int CompareVersions(double v1MajorMinor, Version v2)
		{
			...
			int v1Major = Int32.Parse(segments[0]);
			int v1Minor = segments.Length == 2 ? Int32.Parse(segments[1]) : 0;
		...
		private static int CompareVersions(int v1Major, int v1Minor, int v1Build, int v1Revision, int v2Major, int v2Minor, int v2Build, int v2Revision)
		{
			...
			return v1Revision > v2Revision ? 1 : 0;
		}
