using System.Text;

namespace POCOGenerator.POCOWriters
{
	public abstract class POCOWriter
		: ISnapshot
	{
		private StringBuilder snapshot;

		public void StartSnapshot() => snapshot ??= new StringBuilder();

		public void SnapshotClear() => snapshot?.Clear();

		public void SnapshotWrite(string text) => snapshot?.Append(text);

		public void SnapshotWriteLine(string text = null) => snapshot?.AppendLine(text);

		public StringBuilder EndSnapshot()
		{
			if (snapshot != null)
			{
				StringBuilder sb = snapshot;
				snapshot = null;
				return sb;
			}
			return null;
		}
	}
}
